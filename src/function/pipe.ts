...
// Continue with the pattern for up to 9 functions
...
export function pipe(valueOrFn: any | Function, ...funcs: Function[]): any {
  if (isFunction(valueOrFn)) {
    return (...args: any[]) => {
      return funcs.reduce((acc, fn) => fn(acc), valueOrFn(...args))
    }
  }
  return funcs.reduce((acc, fn) => fn(acc), valueOrFn)
}
